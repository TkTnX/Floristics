...
export const ProductReviews = ({ id, rating }: Props) => {
	...
	return (
		<section className='relative mt-13 -mb-25 bg-white py-25'>
			<div className='border-bg-gold relative container flex flex-col border pr-0! pl-0! lg:flex-row'>
				{data.length > 0 ? (
					...
					<div className='relative z-2 flex w-full flex-col items-center justify-between gap-2 px-4 py-12.5 text-center sm:flex-row sm:px-16 sm:text-left'>
						<p className='text-2xl font-light'>
							На данный момент ещё нет отзывов о товаре
						</p>
						<AddReviewDrawer>
			...
			<Image
				className={cn('absolute -bottom-50 opacity-50', {
					'bottom-0': data.length
				})}
