import { cn, useQueryFilters } from '@/shared'
...
export const SelectedFilters = ({ className }: Props) => {
	...
	const onReset = () => {
		...
		router.replace(`?${emptySearchParams.toString()}`)
	}
	return (
		<div className={cn('flex items-center gap-2.5', className)}>
			...
			<button onClick={onReset} className='text-bg-gold text-sm'>Очистить фильтры</button>
		</div>
	)
}
